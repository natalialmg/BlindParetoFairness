...
def BPF_trainer(dataloader_functional, pd_train, pd_val,
    ...
    while (game < config.GAMES):
        ...
        # losses_gap = np.maximum(loss_a1_train, loss_a0_train) - loss_iw_a1_train
        # best_worst_loss_gap = np.minimum(best_worst_loss_gap, losses_gap)

        # losses_gap = np.maximum(loss_a1_train, loss_a0_train)  #worst loss train
        # best_worst_loss_gap = np.minimum(best_worst_loss_gap, losses_gap) #best worst loss train

        # lr_cooldown += 1
        # if (losses_gap>best_worst_loss_gap) & (game>40) & (lr_cooldown>5):
        #     best_worst_loss_gap = losses_gap #reset gap
        #     if flag_out == 'max_weight_change':
        #         print(flag_out)
        #         BPF_projector.max_weight_change *= 1/config.lrdecay
        #     if flag_out == 'cost_delta_improve':
        #         print(flag_out)
        #         BPF_projector.cost_delta_improve *= 1/config.lrdecay
        #
        #     if (lr_cooldown>5):
        #         print('reduce lr')
        #         lr_cooldown = 0
        #         optimizer.param_groups[0]['lr'] = np.maximum(optimizer.param_groups[0]['lr'] * config.lrdecay, 1e-12)
